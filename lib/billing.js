...
// Called by the Digistore24 webhook handler whenever a purchase
...
// Called when Digistore24 reports a refund, chargeback, or the end of a paid
// billing period (cancellation) — revokes the credits/plan so the customer
// doesn't keep access after the money has been returned or the plan ended.
async function revokePlan({ email, reason }) {
  email = (email || '').toLowerCase().trim();
  ...
  db.prepare('UPDATE users SET credits = 0, expiry_date = ?, updated_at = ? WHERE id = ?')
    .run(nowISO(), nowISO(), user.id);

  await sendEmail(email, 'Your plan has ended',
    `Hi ${user.name},\n\nYour plan access has ended (reason: ${reason}). ` +
    `If you believe this is a mistake, please contact support.\n`);

  return { found: true, id: user.id };
}

module.exports = { grantOrRenewPlan, revokePlan, addDays };
